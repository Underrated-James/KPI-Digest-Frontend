...
const formSchema = z.object({
  ...
  status: z.enum(["active", "inactive", "inProgress", "draft"]),
  finishDate: z.date({
  ...
  return (
    <Card className="w-full border border-border bg-card shadow-xl ring-1 ring-border/70 sm:max-w-md">
      <CardHeader className="border-b border-border/80 pb-5">
        ...
        <CardDescription>
          {viewOnly
            ? "Review this project's details, status, timeline, and assigned members."
            : initialData
        ...
        <form id="project-form" onSubmit={form.handleSubmit(handleSubmit)}>
          <FieldGroup>
            ...
            <Controller
              ...
              render={({ field, fieldState }) => (
                <Field data-invalid={fieldState.invalid}>
                  <FieldLabel htmlFor="project-finish-date">
                    Target Finish Date
                  </FieldLabel>
                  <Input
                    {...field}
