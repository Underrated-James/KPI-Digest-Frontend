...
import {
  closeTicketForm,
  selectEditingTicket,
  selectTicketFormMode,
  selectIsTicketFormOpen,
  selectTicketStatusOverride,
...
import { MarkdownRichTextEditor } from "@/components/editor/markdown-rich-text-editor";
import {
  Select,
  SelectContent,
  SelectItem,
...
function normalizeTicketFormStatus(
...
function ticketToFormValues(ticket: Ticket): TicketFormValues {
  const safeStatus = normalizeTicketFormStatus(ticket.status);
  ...
  return {
    ...
    devTimeSpent:
      ticket.devTimeSpent !== undefined && ticket.devTimeSpent !== null
        ...
        : null,
    testingTimeSpent:
      ticket.testingTimeSpent !== undefined && ticket.testingTimeSpent !== null
...
export function TicketForm() {
  ...
  const formMode = useAppSelector(selectTicketFormMode);
  const statusOverride = useAppSelector(selectTicketStatusOverride);
  const viewOnly = formMode === "view";
  ...
  React.useEffect(() => {
    ...
    const normalizedStatus = normalizeTicketFormStatus(currentStatus);
  ...
  }, [form, isOpen, selectedStatus]);

  React.useEffect(() => {
    if (!isOpen || !editingTicket || !statusOverride || viewOnly) {
      return;
    }

    if (form.getValues("status") !== statusOverride) {
      form.setValue("status", statusOverride, {
        shouldDirty: true,
        shouldValidate: true,
      });
    }
  }, [editingTicket, form, isOpen, statusOverride, viewOnly]);
  ...
  const onSubmit = (values: TicketFormValues) => {
    ...
    if (values.status === "completed") {
      ...
      if (values.devTimeSpent == null) {
        form.setError("devTimeSpent", {
          type: "required",
          message: "Dev spent time is required before completing this ticket",
        });
        hasCompletionError = true;
      }

      if (values.testingTimeSpent == null) {
        form.setError("testingTimeSpent", {
          type: "required",
          message: "QA spent time is required before completing this ticket",
        });
        hasCompletionError = true;
      }
  ...
  const canEditActualTimeSpent =
    isCompletionFlow && !viewOnly && !isEditBlocked;
    ...
    <Card className="mx-auto flex h-full min-h-0 w-full max-w-7xl max-h-[calc(100dvh-0.75rem)] flex-col border border-border bg-card shadow-xl ring-1 ring-border/70 sm:max-h-[calc(100vh-2rem)]">
      ...
      <CardContent className="min-h-0 flex-1 overflow-x-hidden overflow-y-auto px-4 pt-4 sm:px-6 sm:pt-5">
        ...
        <Form {...form}>
          ...
          >
            <div
              className={`mx-auto flex min-h-0 w-full max-w-7xl flex-1 flex-col ${isEditBlocked ? "pointer-events-none opacity-50" : ""}`}
            >
                ...
                <section className="rounded-2xl border border-border/70 bg-card/70 p-4 sm:p-5">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-muted-foreground">
                      Set the project context, sprint, and the main ticket
                      information.
                    </p>
                  </div>
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <Select
                            ...
                            disabled={viewOnly || isEditBlocked}
                            onValueChange={(value) => {
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormDescription>
                            Optional. Tickets can be created before a sprint
                            exists.
                          </FormDescription>
                          <FormMessage />
                        </FormItem>
                  ...
                  <div className="mt-5 grid gap-4 xl:grid-cols-[minmax(0,1.35fr)_minmax(220px,0.8fr)_minmax(220px,0.8fr)]">
                    <FormField
                      control={form.control}
                      name="ticketTitle"
                      render={({ field }) => (
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <Select
                              ...
                              value={normalizeTicketFormStatus(field.value)}
                            >
                              <FormControl>
                                <SelectTrigger>
                      ...
                      <FormItem>
                        ...
                        <div className="flex min-h-10 flex-col gap-2 rounded-md border border-input bg-muted/40 px-3 py-2.5 sm:py-2">
                          ...
                          <FormDescription className="!mt-0 text-xs leading-snug">
                            New tickets start as Open. You can advance the
                            status later from the table or edit mode.
                          </FormDescription>
                        </div>
                      </FormItem>
                    )}
                        ...
                        <FormItem>
                          ...
                          <FormControl>
                            <MarkdownRichTextEditor
                              ...
                              readOnly={viewOnly || isEditBlocked}
                              autoFocus={!editingTicket && !viewOnly}
                              placeholder={
                                viewOnly
                                  ? "No description provided."
                                  : "Write the ticket description here. Markdown is saved automatically."
                              }
                              className="min-h-[280px]"
                            />
                          </FormControl>
                          <FormDescription>
                            Optional. Rich text is stored as markdown so it stays portable and easy to reuse.
                          </FormDescription>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormDescription>
                            Required. Link this ticket back to the source of
                            truth.
                          </FormDescription>
                          <FormMessage />
                        </FormItem>
                ...
                <section className="space-y-6">
                  <div className="rounded-2xl border border-border/70 bg-card/70 p-4 sm:p-5">
                    <div className="mb-4">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Connect the ticket to the people responsible for
                        delivery and QA.
                      </p>
                    </div>
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <FormDescription>
                              Optional. Leave this unassigned if staffing is not
                              finalized yet.
                            </FormDescription>
                            <FormMessage />
                          </FormItem>
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <FormDescription>
                              Optional. QA can remain open until the team is
                              ready.
                            </FormDescription>
                            <FormMessage />
                          </FormItem>
                  ...
                  <div className="rounded-2xl border border-border/70 bg-card/70 p-4 sm:p-5">
                    <div className="mb-4">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Capture expected effort for development and testing.
                      </p>
                    </div>
                  ...
                  {editingTicket ? (
                    <div className="rounded-2xl border border-border/70 bg-card/70 p-4 sm:p-5">
                      <div className="mb-4">
                        ...
                        <p className="text-sm text-muted-foreground">
                          {requiresCompletionEffort
                            ? "Required when marking the ticket as completed."
                            : "Visible here for reference. These fields unlock during the completion flow."}
                        </p>
                      </div>
                            ...
                            <FormItem>
                              ...
                              <FormControl>
                                <Input
                                  ...
                                  step="0.5"
                                  placeholder="e.g. 10"
                                  disabled={!canEditActualTimeSpent}
                                  value={field.value ?? ""}
                                  onChange={(event) =>
                                    field.onChange(
                                      event.target.value === ""
                              ...
                              <FormDescription>
                                Actual development hours used for this ticket.
                              </FormDescription>
                              <FormMessage />
                            </FormItem>
                          )}
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              ...
                              <FormDescription>
                                Actual QA hours used for this ticket.
                              </FormDescription>
                              <FormMessage />
                            </FormItem>
                          )}
